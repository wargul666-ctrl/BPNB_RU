...
class SystemGenerator {
   ...
   *
   * Replace refercnes to 'bpnb_borg', 'Bpnb_borg', and 'BPNB_BORG'
  ...
  replaceFileContents() {
    ...
    const replacements = [
      {
        pattern: new RegExp(/game\.bpnb_borg/g),
        replacement: `game.${this.propName}`
      },
      {
        pattern: new RegExp(/flags\.bpnb_borg/g),
   ...
   * Rename files.
   *
   * Rename files that had bpnb_borg in their name, such as
   * css/bpnb_borg.css.
   */
  renameFiles() {
    glob(`build/${this.packageName}/**/*bpnb_borg*.*`).then(files => {
      files.forEach(oldPath => {
        const file = path.basename(oldPath);
        const directory = path.dirname(oldPath);
        fs.rename(oldPath, `${directory}/${file.replaceAll('bpnb_borg', this.packageName)}`, (err) => {
          if (err) throw err;
        });
      })
    })
  ...
  cleanPackageJson() {
    ...
    fs.rmSync(`build/${this.packageName}/src/generate-bpnb_borg-system.mjs`);
...
inquirer
  ...
  .then((answers) => {
    ...
    // Glob Bpnb_borg's files so that we can process them.
    glob('*', {ignore: ['node_modules/**'] }).then(files => {
      // Copy all files into the build dir.
      generator.copyFiles(files);
      // Replace bpnb_borg name mentions in files.
      generator.replaceFileContents();
      // Rename files that had bpnb_borg in their name.
